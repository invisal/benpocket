...
interface ReplicaSetRawResource {
  metadata?: {
    name?: string;
    namespace?: string;
...
export const ReplicaSetDetail: React.FC<ReplicaSetDetailProps> = ({ payload, isTab = false }) => {
  ...
  const { openDeploymentDetail } = useOpenWorkloadDetail();
  ...
  // Fetch full ReplicaSet and its Pods with React Query caching
  const { data: queryData } = useQuery({
    queryKey: [
      'kuberneter',
      'replicaset-detail-data',
      rawConfigPath,
      cluster,
      payload?.ns,
      payload?.name
    ...
    queryFn: async () => {
      ...
      const [rsRes, podsRes] = await Promise.all([
  ...
  const rawItem = (queryData?.rsItem || payload.rawItem) as unknown as
    ReplicaSetRawResource | undefined;
  ...
  const desired = rawItem?.spec?.replicas ?? payload.desired ?? 0;
  const current = rawItem?.status?.replicas ?? payload.current ?? 0;
  const ready = rawItem?.status?.readyReplicas ?? payload.ready ?? 0;
    ...
    propertiesData.push({
      ...
      value: (
        <span>
          ...
          <span
            onClick={() => {
              if (controlledBy.kind === 'Deployment') {
                openDeploymentDetail(payload.ns, controlledBy.name);
              }
            }}
  ...
  propertiesData.push(
    ...
    },
    {
      id: 'replicas',
      name: 'Replicas',
      value: `Desired: ${desired} | Current: ${current} | Ready: ${ready}`
    },
    {
      id: 'selector',
      name: 'Selector',
