...
export const JobDetail: React.FC<JobDetailProps> = ({ payload, isTab = false }) => {
  ...
  const { openCronJobDetail } = useOpenWorkloadDetail();
  ...
  // Fetch full Job and its Pods with React Query caching
  const { data: queryData } = useQuery({
    queryKey: ['kuberneter', 'job-detail-data', rawConfigPath, cluster, payload?.ns, payload?.name],
    queryFn: async () => {
      if (!cluster || !payload?.ns || !payload?.name) return null;
      const configPathArg = rawConfigPath === 'default' ? undefined : rawConfigPath;
      const [jobRes, podsRes] = await Promise.all([
        window.kuberneter.getResources(configPathArg, cluster, K8S_RESOURCE_KEYS.JOBS, payload.ns),
        window.kuberneter.getResources(configPathArg, cluster, K8S_RESOURCE_KEYS.PODS, payload.ns)
      ]);
      const jobItem = ((jobRes?.items || []) as K8sResource[]).find(
        (i) => i.metadata?.name === payload.name
      );
      const allPods = (podsRes?.items || []) as K8sResource[];

      // Match Pods belonging to this Job
      const matchedPods = allPods.filter((pod) => {
        const ownerRefs = pod.metadata?.ownerReferences || [];
        const isOwner = ownerRefs.some((ref) => ref.kind === 'Job' && ref.name === payload.name);
        const hasJobLabel =
          pod.metadata?.labels?.['job-name'] === payload.name ||
          pod.metadata?.labels?.['batch.kubernetes.io/job-name'] === payload.name;
        return pod.metadata?.namespace === payload.ns && (isOwner || hasJobLabel);
      });
  ...
  if (!payload) {
    return <div className="p-4 text-sm text-zinc-500">No job details available.</div>;
  }

  const pods = queryData?.podsList || payload?.podsList || [];
  const rawItem = (queryData?.jobItem || payload.rawItem) as unknown as JobRawResource | undefined;
  ...
  const desired = rawItem?.spec?.completions ?? payload.desired ?? 1;
  const succeeded = rawItem?.status?.succeeded ?? payload.succeeded ?? 0;
  const failed = rawItem?.status?.failed ?? 0;
    ...
    propertiesData.push({
      ...
      value: (
        <span>
          ...
          <span
            onClick={() => {
              if (controlledBy.kind === 'CronJob') {
                openCronJobDetail(payload.ns, controlledBy.name);
              }
            }}
  ...
  propertiesData.push(
    ...
    },
    {
      id: 'completions',
      name: 'Completions',
      value: `${succeeded}/${desired} (Failed: ${failed})`
    },
    {
      id: 'selector',
      name: 'Selector',
    ...
    },
    {
      ...
      value: (
        ...
        >
          {succeeded >= desired ? 'Complete' : failed > 0 ? 'Failed' : 'Running'}
        </span>
      )
    }
  );
  ...
  return (
    <div className={`flex flex-col gap-4 ${isTab ? 'p-6 h-full overflow-y-auto' : 'flex-1'}`}>
      ...
      <div className="flex flex-col gap-2">
        ...
        <MetricsSection namespace={payload.ns} podNames={targetPodNames} resourceLabel="job" />
      </div>
