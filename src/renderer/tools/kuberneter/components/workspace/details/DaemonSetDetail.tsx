...
interface DaemonSetRawResource {
  metadata?: {
    name?: string;
    namespace?: string;
...
export const DaemonSetDetail: React.FC<DaemonSetDetailProps> = ({ payload, isTab = false }) => {
  ...
  // Fetch full DaemonSet and its Pods with React Query caching
  const { data: queryData } = useQuery({
    queryKey: [
      'kuberneter',
      'daemonset-detail-data',
      rawConfigPath,
      cluster,
      payload?.ns,
      payload?.name
    ...
    queryFn: async () => {
      ...
      const [dsRes, podsRes] = await Promise.all([
  ...
  if (!payload) {
    return <div className="p-4 text-sm text-zinc-500">No daemon set details available.</div>;
  }

  const pods = queryData?.podsList || payload?.podsList || [];
  const rawItem = (queryData?.dsItem || payload.rawItem) as unknown as
    DaemonSetRawResource | undefined;
  ...
  const desired = rawItem?.status?.desiredNumberScheduled ?? payload.desired ?? 0;
  const current = rawItem?.status?.currentNumberScheduled ?? payload.current ?? 0;
  const ready = rawItem?.status?.numberReady ?? payload.ready ?? 0;
  const upToDate = rawItem?.status?.updatedNumberScheduled ?? payload.upToDate ?? 0;
  const available = rawItem?.status?.numberAvailable ?? payload.available ?? 0;

  const nodeSelector =
    Object.entries(rawItem?.spec?.template?.spec?.nodeSelector || {})
      .map(([k, v]) => `${k}=${v}`)
      .join(', ') ||
  ...
  const propertiesData: PropertyItem[] = [
    ...
    {
      id: 'scheduling',
      name: 'Pods / Scheduling',
      value: `${desired} desired, ${current} current, ${ready} ready, ${upToDate} up-to-date, ${available} available`
    },
    {
      id: 'nodeSelector',
      name: 'Node Selector',
      ...
      value: (
        <span
          className={`font-semibold ${
            ready < desired ? 'text-amber-500 animate-pulse' : 'text-emerald-500'
          }`}
        >
          {ready >= desired ? 'Running' : 'Degraded'}
        </span>
      )
    }
  ];
