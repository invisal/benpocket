...
export const LimitRangeDetail: React.FC<LimitRangeDetailProps> = ({ payload, isTab = false }) => {
  ...
  if (!payload) {
    return <div className="p-4 text-sm text-zinc-500">No limit range details available.</div>;
  }
            ...
            <div className="flex flex-col border-y border-border/40 divide-y divide-border/20 bg-surface-2/30">
              {allResources.map((resKey) => {
                ...
                return (
                  <div
                    key={resKey}
                    className="flex items-center justify-between px-3 py-2.5 text-sm hover:bg-surface-3/20 transition-colors"
                  >
