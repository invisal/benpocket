...
/**
 * Cross-window payloads for the "focus toolbar" flow: double-clicking a
 ...
 * main/screen-recorder/windows/focus-toolbar-window.ts) floating over the
 ...
 */
export interface FocusToolbarOpenPayload {
  sourceId: string;
...
export interface FocusToolbarStartPayload extends FocusToolbarOpenPayload {
  /**
   ...
   * the bottom of the primary display -- see focus-toolbar-window.ts.
   */
  targetBounds?: ScreenRect;
}

export interface FocusToolbarRecordingResult {
  ok: boolean;
  error?: string;
}
